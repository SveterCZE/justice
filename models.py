...
def convert_date_to_string(converted_date):
    try:
        if converted_date == None:
            ...
            separated_string = str(converted_date).split("-")
...
class Adresy_v2(db.Model):
    ...
    def __repr__(self):
        ...
        if (self.castobce != None) and (self.castobce != self.obec) and self.castobce != None:
            joined_address += self.castobce + ", "
        if self.psc != None and self.psc != None:
            joined_address += self.psc + " "
        if self.obec != None and self.obec != None:
            joined_address += self.obec
        if (self.stat != "Česká republika") and (self.stat != "Česká republika - neztotožněno") and (self.stat != None):
